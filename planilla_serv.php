...
session_start(); 
	  ...
	  @include("include/pdf.php");
		   ...
		   if (isset($_GET['id_planilla'])) {
            ...
            $pdf->formulario($plan['contrib'], $plan['rif'], $plan['vehiculo'], $plan['muelle'], $plan['fec_arribo'], $plan['fec_salida'], $plan['hora'], date("d/m/Y"));
            $pdf->hab_aduana($vis['fecha_v'], $vis['hora_v_ini'], $vis['hora_v_fin'], $vis['total_v_horas'], $vis['total_v_bs'], $vis['total_v_ut'],
                             $ope['fecha_o'], $ope['hora_o_ini'], $ope['hora_o_fin'], $ope['total_o_horas'], $ope['total_o_bs'], $ope['total_o_ut'],
                             $alc['fecha_a'], $alc['hora_a_ini'], $alc['hora_a_fin'], $alc['total_a_horas'], $alc['total_a_bs'], $alc['total_a_ut'],
                             $des['fecha_d'], $des['hora_d_ini'], $des['hora_d_fin'], $des['total_d_horas'], $des['total_d_bs'], $des['total_d_ut'],
                             $otros['fecha_otros'], $otros['puerto'], $otros['total_bs_otros'], $otros['desayuno'], $otros['almuerzo'], $otros['cena'], $plan['total_p']
                    );
            $pdf->relleno();
            $pdf->Output();   
			  ...
			  <label >Atraco a las: </label>
                ...
                <fieldset class="login">
                    ...
                    <input type="hidden" name="idplan" value="<?php if(isset($plan['id_planilla'])) echo $plan['id_planilla']; else echo '0';  ?>" />
                     <input type="hidden" name="ut" value="<?php echo $plan['unit_tributa']; ?>" />
                    <input type="hidden" name="editar" value="true" />
		<div style=" padding-top:15px; text-align:center;">
                    <a href="javascript:submitform();" class="submit">Modificar</a>
                    <a href="consulta_us.php" class="submit">Regresar</a>
                </div>
	</fieldset>
             </form>
             </div>
